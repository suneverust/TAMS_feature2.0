...
const int TAMSBANDWIDTH = 8;
const int FEATURE_SIZE = TAMSBANDWIDTH*TAMSBANDWIDTH;

namespace tams{
/** \brief ADD a new feature type */
struct TAMSFeatureType
{
    float tams_sei_sh[FEATURE_SIZE];
...
operator << (std::ostream& os, const TAMSFeatureType& p)
{
    for (int i = 0; i < FEATURE_SIZE; ++i)
        os << (i == 0 ? "(" : "") << p.tams_sei_sh[i] << (i < (FEATURE_SIZE-1) ? ", " : ")");
    return (os);
}
}
...
POINT_CLOUD_REGISTER_POINT_STRUCT ( tams::TAMSFeatureType,
                                    (float[FEATURE_SIZE], tams_sei_sh, tams_sei_sh))
